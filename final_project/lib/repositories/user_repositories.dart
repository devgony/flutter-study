...
class UserRepository {
  ...
  Future<List<MoodModel>> likedMoods(String userId) async {
    final DocumentSnapshot<Map<String, dynamic>> snapshot =
        await _db.collection("users").doc(userId).get();
    final iterable = Stream.fromIterable(
      List.castFrom<dynamic, String>(snapshot.get("likedMoods")),
    ).asyncExpand((moodId) async* {
      final snapshot = await _db.collection("moods").doc(moodId).get();

      final mood = fromSnapshotToMoodModel(snapshot, userId);
      if (mood != null) {
        yield mood;
      }
    });
    final moods = await iterable.toList();
